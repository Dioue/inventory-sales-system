...
document.addEventListener('DOMContentLoaded', () => {
    const activityLogBtn = document.querySelector('#all-activity-log-btn');
    const activityModal = document.querySelector('#all-activity-log');
    const acitivityLogHide = document.querySelector('#all-activity-log-hide');
    ...
    activityLogBtn.addEventListener('click', async (event) => {
        try {
            ...
            if(activityLogTable){
                ...
                activityLogTable.insert({ data: log });
            }

            const handleModalHide = () => {
                new Modal(activityModal, modalOptions).hide();
                acitivityLogHide.removeEventListener('click', handleModalHide);
            };

            acitivityLogHide.addEventListener('click', handleModalHide, { once: true });
            new Modal(activityModal, modalOptions).show();
        } catch (err) {
            console.error('Error loading activity log:', err);
            alert('Could not load activity log data.');
        }
    })


    if (document.getElementById("all-activity-log-table") && typeof simpleDatatables.DataTable !== 'undefined') {
        ...
        activityLogTable = new simpleDatatables.DataTable("#all-activity-log-table", {
            searchable: true,
            perPage: 5,
            perPageSelect: [5, 10],
